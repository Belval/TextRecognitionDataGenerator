...
def _apply_func_distorsion(image, vertical, horizontal, max_offset, func):
    ...
    horizontal_offsets = [
        func(i)
        for i in range(img_arr.shape[0] + ((max(vertical_offsets) - min(min(vertical_offsets), 0)) if vertical else 0))
    ]
    ...
    if horizontal:
        ...
        for i, o in enumerate(horizontal_offsets):
            if vertical:
                new_img_arr_copy[i, max_offset + o : row_width + max_offset + o, :] = new_img_arr[
                    i, max_offset : row_width + max_offset, :
                ]
            else:
...
def sin(image, vertical=False, horizontal=False):
    ...
    return _apply_func_distorsion(
        image, vertical, horizontal, max_offset, (lambda x: int(math.sin(math.radians(x)) * max_offset))
    )
...
def cos(image, vertical=False, horizontal=False):
    ...
    return _apply_func_distorsion(
        image, vertical, horizontal, max_offset, (lambda x: int(math.cos(math.radians(x)) * max_offset))
    )
